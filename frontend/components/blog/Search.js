...
const Search = () => {
    ...
    const searchSubmit = e => {
        ...
        listSearch({ search }).then((data => {
            setValues({
                ...values, results: data, searched: true, message
                    : `${data.length} blogs found`
            })
        }))
    }

    const handleChange = e => {
        setValues({ ...values, search: e.target.value, serched: false, results: [] })
    }
                ...
                {results.map((blog, i) => {
                    return (<div key={i}>
                        <Link href={`/blogs/${blog.slug}`}>
                            <a className="text-primary">{blog.title}</a></Link>
                    </div>)
                })}
            </div>
        )
